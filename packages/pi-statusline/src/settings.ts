    ...
    if (!isRecord(value.segmentText)) {
      ...
      for (const [name, presentation] of Object.entries(value.segmentText)) {
        ...
        const knownFields = new Set([
          "prefix",
          "suffix",
          ...(name === "model" ? ["truncationLength", "truncationSymbol", "truncationDirection"] : []),
        ]);
  ...
  if (value.extensionStatusIcons !== undefined) {
    if (!isRecord(value.extensionStatusIcons)) {
      ...
      for (const [legacyKey, canonicalKey] of Object.entries(LEGACY_STATUS_ICON_KEYS)) {
        ...
        const inheritedIcon =
          typeof canonicalIcon === "string" ? canonicalIcon : typeof legacyIcon === "string" ? legacyIcon : undefined;
...
export function loadStatuslineSettingsForAgent(agentDir = getAgentDir()): LoadedStatuslineSettings {
  ...
  return isMissingStatuslineSettings(legacy) ? canonical : migrateLegacySettings(canonicalPath, legacy);
}

function migrateLegacySettings(canonicalPath: string, legacy: LoadedStatuslineSettings): LoadedStatuslineSettings {
...
function isMissingStatuslineSettings(settings: LoadedStatuslineSettings): boolean {
  return settings.source === "built-in" && settings.rawDocument === undefined && settings.diagnostics.length === 0;
}
...
export function removeStatuslineSettingsDocumentIfMatches(
  ...
  const quarantinePath = join(dirname(settingsPath), `.${SETTINGS_FILE_NAME}.${randomUUID()}.rollback`);
...
export function readStatuslineSettings(settingsPath?: string): StatuslineConfig {
  return settingsPath ? loadStatuslineSettings(settingsPath).config : loadStatuslineSettingsForAgent().config;
}
...
function normalizePalette(value: unknown, config: StatuslineConfig, diagnostics: StatuslineConfigDiagnostic[]) {
  ...
  if (typeof value === "string") {
    if (!(PALETTE_NAMES as readonly string[]).includes(value)) {
      diagnostics.push(
        invalidDiagnostic("palette", `Expected a palette object or one of: ${PALETTE_NAMES.join(", ")}`),
      );
      return;
    }
...
function normalizeModelTruncation(
  ...
  if (length !== undefined) {
    if (typeof length !== "number" || !Number.isInteger(length) || length < 0 || length > 1000) {
      diagnostics.push(invalidDiagnostic(`${path}.truncationLength`, "Expected an integer from 0 through 1000"));
  ...
  if (direction !== undefined) {
    if (
      ...
      diagnostics.push(
        invalidDiagnostic(`${path}.truncationDirection`, `Expected one of: ${TRUNCATION_DIRECTIONS.join(", ")}`),
      );
    } else {
      config.segmentText.model.truncationDirection = direction as (typeof TRUNCATION_DIRECTIONS)[number];
    }
  }
}

function isSafeSegmentText(value: unknown, path: string, diagnostics: StatuslineConfigDiagnostic[]): value is string {
  if (typeof value !== "string") {
...
function normalizeEnum<K extends "palettePreset" | "density" | "separator", T extends StatuslineConfig[K]>(
  ...
  if (typeof candidate !== "string" || !accepted.includes(candidate as T)) {
    diagnostics.push(invalidDiagnostic(field, `Expected one of: ${accepted.map(String).join(", ")}`));
    return;
  }
...
function cloneSegmentPalette(palette: SegmentPalette): SegmentPalette {
  return Object.fromEntries(Object.entries(palette).map(([name, colors]) => [name, { ...colors }])) as SegmentPalette;
}
...
function blockingDiagnostics(diagnostics: readonly StatuslineConfigDiagnostic[]): StatuslineConfigDiagnostic[] {
