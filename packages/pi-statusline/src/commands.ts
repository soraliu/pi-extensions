...
async function chooseSegments(ctx: ExtensionCommandContext, options: StatuslineCommandOptions) {
  ...
  await ctx.ui.custom((tui, theme, keybindings, done) => {
    const commit = (name: SegmentName, change: { nextDocument: string; previousDocument: string }): boolean => {
      try {
    ...
    const renderList = (width: number): string[] => {
      ...
      const startIndex = Math.max(
        0,
        Math.min(selectedIndex - Math.floor(viewportSize / 2), Math.max(0, names.length - viewportSize)),
      );
      ...
      for (let index = startIndex; index < endIndex; index += 1) {
        ...
        if (section !== previousSection) {
          const heading = section === "visible" ? "Visible — render order" : "Hidden — not rendered";
      ...
      if (startIndex > 0 || endIndex < names.length) {
        lines.push(theme.fg("dim", truncateToWidth(`  (${startIndex + 1}-${endIndex}/${names.length})`, safeWidth)));
      }
...
function palettePresetDocument(current: LoadedStatuslineSettings, palettePreset: PalettePreset): string {
  const { parsed } = editableSettings(current, "choosing a palette preset");
  if (palettePreset === "custom" && !isRecord(parsed.palette)) {
    const seedPreset = isPaletteName(current.config.palettePreset) ? current.config.palettePreset : "tokyo-night";
...
function segmentControlHints(
  ...
  return width < 30
    ? ["↑↓ navigate", "Enter/Space toggle", "M move · B line break", "Alt+↑/↓ quick move", `${closeKeys} close`]
    : [
...
function restoreStatuslineSettings(
  ...
  try {
    ...
    removeStatuslineSettingsDocumentIfMatches(options.settingsPath, saved.rawDocument, saved.fileIdentity);
    if (previous.rawDocument === undefined || previous.settingsPath !== options.settingsPath) {
      options.apply(previous, ctx);
      return undefined;
    }
    const restored = (options.save ?? saveStatuslineSettingsDocument)(options.settingsPath, previous.rawDocument);
...
function editableSettings(
  ...
  const rawDocument =
    current.source === "user" && current.rawDocument !== undefined
      ? current.rawDocument
      : current.source === "built-in" && current.rawDocument === undefined && current.diagnostics.length === 0
        ? DEFAULT_STATUSLINE_DOCUMENT
        : undefined;
