...
function buildSegment(
  ...
  switch (name) {
    ...
    case "branch": {
      ...
      return segment(name, formatGitBranchValue(branch, runtime.gitStatus, pr), config, "accent", "git");
    }
    case "cwd":
      return segment(
        name,
    ...
    case "context": {
      ...
      const percentage = usage?.percent === null || usage?.percent === undefined ? "?" : `${usage.percent.toFixed(1)}%`;
