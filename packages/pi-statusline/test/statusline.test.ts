...
test("statusline does not render stale in-flight git status after a branch change", async () => {
  ...
  async function execGitStatus(_command: string, args: string[], options?: { signal?: AbortSignal }) {
    if (args[0] === "rev-parse") {
      return { stdout: "/workspace\n", stderr: "", code: 0, killed: false };
    }
...
test("formatToolActivity shows only active or streaming work", () => {
  ...
  assert.equal(formatToolActivity(runtime({ activeTools: new Map([["read", 2]]), isStreaming: false })), "⚙️ read×2");
  assert.equal(formatToolActivity(runtime({ activeTools: new Map(), isStreaming: true })), "💭 thinking");
  assert.equal(formatToolActivity(runtime({ activeTools: new Map(), isStreaming: false })), undefined);
});
...
test("prLinkFromStatuses keeps the linked PR token and drops the tail and non-PR states", () => {
  const link = "\x1b]8;;https://github.com/o/r/pull/123\x07#123\x1b]8;;\x07";
  assert.equal(prLinkFromStatuses(new Map([["github-pr", `PR ${link}: checks failing (1), approved`]])), link);
...
test("prContextFromStatuses chooses one actionable state by precedence", () => {
  ...
  const context = (details: string) => prContextFromStatuses(new Map([["github-pr", `PR ${link}: ${details}`]]));
  ...
  assert.equal(prContextFromStatuses(new Map([["github-pr", "PR #123\u001b[31m: checks passing"]])), undefined);
...
test("git branch text includes compact status before PR link", () => {
  ...
  assert.equal(
    formatGitBranchText("feature", { ahead: 1, behind: 0, staged: 3, modified: 0, untracked: 2, conflicts: 0 }, link),
    `🌿 feature ⇡1 +3 ?2 (${link})`,
  );
...
test("statusline settings load extension icon overrides", () => {
  ...
  writeFileSync(settingsPath, JSON.stringify({ extensionStatusIcons: { goal: "", caffeinate: "☕", bad: 1 } }));
...
test("statusline settings read legacy files and prefer the canonical package filename", async () => {
  ...
  try {
    ...
    writeFileSync(legacyPath, JSON.stringify({ extensionStatusIcons: { goal: "🎯" }, futureOption: true }));
  ...
  try {
    ...
    fs.readFileSync = ((...args: Parameters<typeof fs.readFileSync>) => {
      ...
      if (createCanonical && String(args[0]) === legacyPath) {
        ...
        writeFileSync(canonicalPath, JSON.stringify({ extensionStatusIcons: { goal: "canonical" } }));
      }
      return result;
...
test("extension status icons use config, leading emoji, defaults, and fallback", () => {
  ...
  assert.equal(formatExtensionStatus("caffeinate", "☕ display", theme, config({ caffeinate: "🍵" })), "🍵 display");
...
test("statusline render uses installed package id icon aliases from settings", async () => {
  ...
  try {
    ...
    writeFileSync(join(cwd, ".pi", "settings.json"), JSON.stringify({ packages: ["npm:@vendor/pi-foo@1.2.3"] }));
...
test("extension status icons match arbitrary exact keys and explicit namespace wildcards", () => {
  ...
  assert.equal(
    formatExtensionStatus("foo:server", "running", theme, config({ "foo:*": "🧪", "foo:server": "🖥️" })),
    "🖥️ running",
  );
  assert.equal(
    formatExtensionStatus("foo:server:worker", "running", theme, config({ "foo:*": "🧪", "foo:server:*": "⚙️" })),
    "⚙️ running",
  );
  assert.equal(formatExtensionStatus("foo:worker", "running", theme, config({ "foo:*": "🧪" })), "🧪 running");
  ...
  for (const key of ["foo", "foobar", "foo/server"]) {
    assert.equal(formatExtensionStatus(key, "running", theme, config({ "foo:*": "🧪" })), "🔌 running");
  }
});
...
test("extension status icons bridge canonical and legacy sync and retry keys", () => {
  ...
  assert.equal(
    formatExtensionStatus("retry", "retrying", theme, config({ "unknown-error-retry": "CUSTOM-RETRY" })),
    "CUSTOM-RETRY retrying",
  );
  assert.equal(formatExtensionStatus("pisync", "pushing", theme, config({ sync: "NEW-SYNC" })), "NEW-SYNC pushing");
  assert.equal(
...
test("extension status icons use installed package id aliases without fuzzy matching", () => {
  ...
  assert.deepEqual(
    [...aliases],
    [
      ["foo", ["npm:@vendor/pi-foo@1.2.3", "npm:@vendor/pi-foo", "@vendor/pi-foo", "pi-foo", "foo"]],
      ["bar", ["@vendor/bar", "bar"]],
    ],
  );
  assert.equal(
  ...
  assert.equal(
    formatExtensionStatus("foo/server", "running", theme, config({ "npm:@vendor/pi-foo@1.2.3": "📦" }), aliases),
    "📦 running",
  );
  assert.equal(formatExtensionStatus("bar", "running", theme, config({ "@vendor/bar": "🍫" }), aliases), "🍫 running");
  assert.equal(
...
test("extension status icon aliases preserve exact-key precedence and skip ambiguous packages", () => {
  ...
  assert.equal(
    formatExtensionStatus("foo:server", "running", theme, config({ "@first/pi-foo": "1️⃣" }), aliases),
    "🔌 running",
  );
  ...
  assert.equal(
    formatExtensionStatus("foo:server", "running", theme, config({ "@vendor/pi-foo": "" }), unambiguousAliases),
    "running",
  );
});

test("long extension status lines wrap to terminal width without ellipsis", () => {
  const lines = wrapExtensionStatusline("🔎 PR #123: checks pending (12) changes requested 45 comments", 30);
