...
test("line breaks may repeat when separated but consecutive line breaks are invalid", () => {
  ...
  assert.deepEqual(multiline.config.segments, ["model", "line_break", "cwd", "line_break", "branch"]);
...
test("all named palettes, separators, empty segments, and environment independence are accepted", () => {
  ...
  try {
    for (const palettePreset of ["tokyo-night", "ocean", "sunset", "forest", "candy", "neon", "mono"]) {
      assert.equal(normalizeStatuslineConfig({ palettePreset }).config.palettePreset, palettePreset);
      assert.equal(normalizeStatuslineConfig({ palette: palettePreset }).config.palettePreset, palettePreset);
    }
...
test("invalid recognized fields are rejected on save while unknown fields remain allowed", () => {
  ...
  const path = settingsFilePath(root);
  try {
    assert.throws(() => saveStatuslineSettingsDocument(path, JSON.stringify({ palette: "bad" })), /palette/i);
    assert.throws(
      () => saveStatuslineSettingsDocument(path, JSON.stringify({ palettePreset: "bad" })),
      /palettePreset/i,
    );
    assert.throws(
      () => saveStatuslineSettingsDocument(path, JSON.stringify({ palette: { time: { bg: "#abcd" } } })),
      /palette\.time\.bg/i,
    );
    assert.throws(
      () =>
    ...
    assert.throws(
      () => saveStatuslineSettingsDocument(path, JSON.stringify({ segmentText: { model: { suffix: "\n\n" } } })),
      /use line_break/iu,
    );
